...
probabilities = [0.013, 0.03123, 0.105, 0.2, 0.29, 0.24, 0.12]
days = [1, 2, 3, 4, 5, 6, 7]


def tweak_probabilities(probabilities, variation):
    def twk_p(p):
...
def get_day_n_transaction_df(days, probabilities, variation, sample_size, max_value_coef):
    probabilities = tweak_probabilities(probabilities, variation=variation)
    frequencies = get_frequencies(sample_size, probabilities)

    week_day_list = get_week_day_list(days, frequencies)
    transaction_list = get_transaction_list(sample_size, max_value_coef)
...
for fond in [fond_1k, fond_ltv, fond_BnY]:
    ...
    df_days_n_trans = get_day_n_transaction_df(days,
                                               probabilities, variation=0.2,
                                               sample_size=sample_size, max_value_coef=max_value_coef)
