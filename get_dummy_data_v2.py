...
Code schame:
For each Fond:
    A fond object is generated. It contains attribuse of:
        name
        desired total sample size
        desired maximum size of single transaction (see comment get_transation_list() )
    For each week (in defined date range):   
            Two dataframes - of transactions and dates - are generated and later joined together
                Transaction df is generated by:
                    Making weekday list:
                        raw_probabilities - probability of weekday in the week
                        Taking raw_probabilities with desired distribution and tweaking it a bit 
                            to add more variability for weekday distribution.
                        These probabilities are turned into frequencies of each weekday in the desired weekly_sample_size
                            (the defined weekly_sample_size is also randomized (+/-50%)
                        Finally, each weekday is multiplied by its frequency to get a list of weekdays. This
                            gives a list of weekdays that represent multiple transactions
                    Making weekly transaction list:
                        This is created by creating random numbers in exponential distribution
                    Creating weakly transaction df out of these two lists. To do that, the lengths of these list are equaled
                Dates df is generated by:
                    Making a list of dates in defined range
                    Deriving week numbers and weekdays from the date
            These dataframes are later joined and concated with dataframes of previously generated weeks and funds 
